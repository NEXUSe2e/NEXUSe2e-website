...
To externalize the logging configuration (this is handled slightly differently as this is a library we include), you need to make the following changes:
  Place a copy of the log4j.properties file from the configuration folder in an external folder (see above, this is done exactly the same as for the other configuration files, and we advice to simply use the same external folder).
  Navigate to C:\Program Files\NEXUSe2e\tomcat\bin\, launch NEXUSe2ew.exe 
  .Add '-Dexternalconfig=file:/C:/Example/NEXUSe2e/log4j.properties' under Java Options.
*/
</script>